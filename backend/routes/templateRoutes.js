...
router.get('/event', async (req, res) => {
  ...
  try {
    ...
    if (!template) {
      return res.status(404).json({ error: `Template with event "${event}" not found` });
    }
...
router.post('/', async (req, res) => {
  const { title, emailSubject, status, emailBody, type, event, description } = req.body;

  if (!title) {
    return res.status(400).json({ error: 'Title, emailBody, and type are required' });
  }

  try {
    const newTemplate = new Template({ title, emailSubject, status, emailBody, type, event, description });
