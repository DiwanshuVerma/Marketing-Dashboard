...
const Template = require('../models/Template'); // Your schema
...
async function sendEmail(templateType, recipientEmail, placeholders = {}) {
  try {
    ...
    const template = await Template.findOne({ event: templateType, status: 'Active' });
    if (!template) throw new Error(`Template of type '${templateType}' not found`);
